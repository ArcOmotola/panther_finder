...
if (isset($_POST['submit'])) {
    ...
    if ($item_name == "" || $item_address == "" || $item_phone == "" || $time_found == "" || $description == "" || $visibility == "") {
        ...
        header("Location: ../climant.php?error=" . $error_message);
        exit;
    } else {
        ...
        try {
            ...
            // }
            $register_sql = "INSERT INTO claimant_reports (user_id,title,description,image,phone,category_id,color_id,visibility) VALUES (:user_id,:title,:description,:image,:phone,:category_id,:color_id,:visibility)";
            $params = [
                'user_id' => $user_id,
                'title' => $item_name,
            ...
            if (!$insert) {
                ...
                header("Location: ../climant.php?success=" . $success_message);
            }
        } catch (\Throwable $th) {
            // log($th->getMessage());
